...
def task():
    ...
    if request.method == 'GET':
        query = request.args
        data = db.tasks.find_one({'_id': ObjectId(query['id'])})
    ...
    if request.method == 'POST':
        ...
        if data['ok']:
            db_response = db.tasks.insert_one(data['data'])
            return_data = db.tasks.find_one(
    ...
    if request.method == 'DELETE':
        if data.get('id', None) is not None:
            db_response = db.tasks.delete_one(
...
def list_tasks():
    ...
    if request.method == 'GET':
        query = request.args
        data = db.tasks.find({'email': user['email']})
